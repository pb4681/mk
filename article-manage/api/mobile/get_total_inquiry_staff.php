...
$str = "WHERE deliveruser='$user'";
if(isset($_GET['status'])){
    $status = inject_check($_GET['status']);
    $str = "WHERE status='$status' AND deliveruser='$user'";
}
if(isset($_GET['offset'])){
    $offset = inject_check($_GET['offset']);
    $pre = $offset;
}else{
    $pre = 0;
}

$res = $db->query("SELECT * FROM inquiry $str ORDER BY id DESC LIMIT $pre,20");
$res1 = $db->query("SELECT COUNT(id) FROM inquiry WHERE staffclick=0 AND deliveruser='$user'");
...
while($row = mysql_fetch_assoc($res)){
    array_push(
        $arr,array(
            ...
            'status'=>$row['status'],
            'area'=>$row['area'],
            'staffclick'=>$row['staffclick'] == 0 ? true : false
        )
    );
}
